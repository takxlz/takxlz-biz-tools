...
// ---- CLI: フラグ配線（--mark / --header） ------------------------------------

const SCRIPT_PATH = path.join(__dirname, 'tsv-mark-matches.js');

// CLI を子プロセスで実行し、標準出力（データ）を返す。入力は stdin（- 指定）で渡す。
function runCli(args, stdin) {
  return execFileSync('node', [SCRIPT_PATH, ...args], { input: stdin, encoding: 'utf8' });
}

test('CLI: --mark と --header で印・追加列名を CLI から変更できる', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tsv-mark-'));
  const listPath = path.join(tmpDir, 'list.txt');
  fs.writeFileSync(listPath, 'app.properties\n');
  const input = 'file\tkey\nsrc/app.properties\tk1\nsrc/other.properties\tk2\n';

  const out = runCli(['-', listPath, '-c', 'file', '--mark', 'X', '--header', '一致'], input);

  assert.strictEqual(
    out,
    'file\tkey\t一致\nsrc/app.properties\tk1\tX\nsrc/other.properties\tk2\t\n'
  );

  fs.rmSync(tmpDir, { recursive: true, force: true });
});

test('CLI: --mark/--header 未指定なら既定（match / ●）を使う', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tsv-mark-'));
  const listPath = path.join(tmpDir, 'list.txt');
  fs.writeFileSync(listPath, 'app.properties\n');
  const input = 'file\tkey\nsrc/app.properties\tk1\n';

  const out = runCli(['-', listPath, '-c', 'file'], input);

  assert.strictEqual(out, 'file\tkey\tmatch\nsrc/app.properties\tk1\t●\n');

  fs.rmSync(tmpDir, { recursive: true, force: true });
});

test('CLI: 短縮形 -m / -H でも印・追加列名を変更できる', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tsv-mark-'));
  const listPath = path.join(tmpDir, 'list.txt');
  fs.writeFileSync(listPath, 'app.properties\n');
  // 最左列が判定対象（既定）なので -c は不要
  const input = 'file\nsrc/app.properties\n';

  const out = runCli(['-', listPath, '-m', '*', '-H', 'hit'], input);

  assert.strictEqual(out, 'file\thit\nsrc/app.properties\t*\n');
